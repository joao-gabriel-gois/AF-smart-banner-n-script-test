// I couldn't achieve an implementation where banners are encapsulated, as I did for smart
// script. As Banenrs rely on at least one request that, consumes a lotvof data set in window.AF
// by AppsFlyer code snippet*, in order to properly render them. I just created this export to
// be able to properly call it whenever the importing context want (for this case, it must be
// triggered by the DOMContentLoaded event, once this page is fetching only one entry-point JS
// Script - index.js - in the HTML header).
...
export default {
  show: () => AF('banners', 'showBanner'),
  hide: () => AF('banners', 'hideBanner'),
}
